...
from typing import TypedDict, Iterator, Dict, List, Tuple, Union, Callable
...
## Functions
def search_node(root: ET.Element, node: str, child_element_names: List[str]) -> Dict[str, str]:
    """
    ...
    values = {
        ...
        child.replace("_", r"\_"): txt
        for child in child_element_names
        if (txt := root.findtext(f"{node}/{child}")) is not None
    }
    return values
...
def detrend(pivots, values, periods: List[float]) -> pd.Series:
    ...
    masks = np.ones(len(values), dtype=np.bool)  # type: ignore
    ...
    for period, coefs in wave_coef_map.items():
      A = coefs['sin']
      B = coefs['cos']
      s, C = ((z:=np.arctan2(B, A)), A / np.cos(z))  # type: ignore
...
def add_plot_table(fig: plt.Figure, axes: List[plt.Axes], info: Dict[str, str]) -> None:
    ...
    if 22 < len(info['Fourier']) < 27:
        tab.get_celld()[(5, 0)].set_text_props(fontproperties=FontProperties(size=9))
    elif len(info['Fourier']) >= 27:
        fourier_ele = info['Fourier'].split(', ')
        fourier_len = np.array([len(ele) for ele in fourier_ele])
        which_max = np.argmin(fourier_len.cumsum() < 35)
...
def load_data(args: argparse.Namespace) -> Callable[[], Iterator[Tuple[int, pd.DataFrame, pd.DataFrame]]]:
    """
    ...
    doi = TRAIN_DIR.joinpath(args.case, args.state)
    sim_path = TRAIN_DIR.joinpath(args.case, args.state, "synthetic.csv")
    sim_dat = pd.read_csv(sim_path, engine='c', memory_map=True)
    meta_path = doi.joinpath("romMeta.xml")
    ...
    epri_dict = {
        ...
        year: transform_data(doi.joinpath(f"Data_{i+3}.csv"))
...
def generate_plots(func: Callable[[plt.Axes, pd.DataFrame, pd.DataFrame], None],
    ...
    for i, (year, year_rom, epri_dat) in enumerate(fetch()):
        ...
        ax.set_title(f"{year} {args.state}-Total Load")
...
def write_image(fig: plt.Figure, args: argparse.Namespace, **kwargs) -> None:
    ...
    xmlfile = TRAIN_DIR/args.case/f"{args.state.lower()}_{args.case}_train.xml"
    func_name = func.__name__.split("_")[1] if (func:=kwargs.get('func', False)) else ''
    fig_name = f"{func_name}_{args.state.lower()}_{xmlhash}.png"

    if args.subparser == 'single-plot':
        fig_name = f"{args.function}si_{args.state.lower()}_{xmlhash}.png"

    fig_path = TRAIN_DIR/args.case/fig_name
    ...
    fig_dir = TRAIN_DIR.joinpath(args.case, "figures")
    if args.embed_xml:
        embed_xml(
            xmlfile,
            fig_path,
            # Hacky way to split the name to add an 'em' before first '_'.
            fig_dir/"_".join([(fn := fig_name.split("_"))[0] + "em"] + fn[1:])
        )
...
def initialize(args):
    ...
    """
    xml_file = TRAIN_DIR/args.case/f"{args.state.lower()}_{args.case}_train.xml"
    info = parse_xml(xml_file)
    xmlhash = hash_xml(xml_file)
    # Load up plotting data once to save time
    fetch = load_data(args)
    burn_plots(xml_file)
